...
AddressController.prototype.showMulti = function(req, res) {
  var self = this;
  var options = {
    noTxList: parseInt(req.query.noTxList)
  };
  ...
  let rsp = []
  async.eachSeries(req.addrs, function(addr, callback) {
    self.getAddressSummary(addr, options, function(err, data) {
